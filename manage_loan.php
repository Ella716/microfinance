...
<div class="container-fluid">
	...
	<form action="" id="loan-application">
				...
				<select name="borrower_id" id="borrower_id" class="custom-select browser-default select2">
					<option value=""></option>
						<?php while($row = $borrower->fetch_assoc()): ?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($borrower_id) && $borrower_id == $row['id'] ? "selected" : '' ?>><?php echo $row['name'] . ' | ID Number:'.$row['id_number'] ?></option>
						<?php endwhile; ?>
				</select>
			</div>
			<div class="col-md-6">
		...
		<div class="form-group col-md-6">
			...
			<input type="number" name="amount" class="form-control text-right" step="any" id="inputLam" min="10000" max="40000" value="<?php echo isset($amount) ? $amount : '' ?>">
		</div>
		</div>
		<div class="row">
			<div class="form-group col-md-6">
			<label class="control-label">Purpose</label>
			<textarea name="purpose" id="inputPrse" cols="30" rows="2" class="form-control"><?php echo isset($purpose) ? $purpose : '' ?></textarea>
		</div>
...
<script>
	...
	$('#loan-application').submit(function(e){
		e.preventDefault()
		start_load()
		var bid = $('#borrower_id').val();
		var ltid = $('#loan_type_id').val();
		var pid = $('#plan_id').val();
		var amnt = $('#inputLam').val();
		var purpse = $('#inputPrse').val();
		if(bid.trim() == '' ){
			...
			alert('Please enter the purpose of this loan.');
			$('#inputPrse').focus();
			return false;
		}else{
		$.ajax({
